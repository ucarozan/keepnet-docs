...
### **Double Barrel Settings**

When a campaign includes one or more **Double Barrel** scenarios, a **Double Barrel Settings** section appears on the **Delivery Settings** page. It controls how the Lure and Payload emails are delivered.

<table><thead><tr><th width="169.171875">Setting</th><th>Description</th></tr></thead><tbody><tr><td>Send Order</td><td>Which email is sent first — <strong>Lure First</strong> (default) or <strong>Payload First</strong>.</td></tr><tr><td>Delay Between Emails</td><td>The wait time between the two emails: 5, 10, 15, 30, 60, 120, or 240 minutes (default 60 minutes).</td></tr><tr><td>Urgent Flag</td><td>Adds the email "urgent" flag to one or both messages — <strong>None</strong>, <strong>Lure Only</strong>, <strong>Payload Only</strong>, or <strong>Both</strong>.</td></tr><tr><td>Skip payload if reported</td><td>When enabled, the Payload email is not sent to users who reported the Lure email. Enabled by default.</td></tr><tr><td>Responsive delivery</td><td>When enabled, the second email is sent only to users who opened the first one. Disabled by default.</td></tr></tbody></table>
