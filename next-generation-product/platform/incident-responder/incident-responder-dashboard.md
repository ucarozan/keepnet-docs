...
<table data-header-hidden><thead><tr><th width="150"></th><th width="581.7800776196638"></th></tr></thead><tbody><tr><td>Details</td><td><p>This is the area where the details of the email are shown. See the following information:</p><ul><li>Analysis date of the reported email</li><li>From</li><li>From Name</li><li>To</li><li>CC</li><li>BCC</li><li>The date and time the user received the email</li><li>Sender IP of the email</li><li>The name of the folder where the email is located</li><li>The number of attachments</li><li>The number of URLs</li><li>The location of the sender IP address.</li></ul><p><br>At the same time, the email server IP address to which the email is sent, blacklist control is performed in analysis services. You can see the results on this screen under the <strong>Sender</strong> <strong>IP</strong> <strong>Blacklist</strong> <strong>Check</strong> title.</p></td></tr><tr><td>Header</td><td>The header information of the email is displayed in this field.</td></tr><tr><td>Email Preview</td><td>The preview of the email is shown in this area.</td></tr><tr><td>URLs</td><td>URLs and their analysis results in the email are displayed in this field. If the main domain redirects, each subsequent domain in the redirection chain is extracted and scanned until the final destination.</td></tr><tr><td>Attachments</td><td>The name of the attachment files and their hash information as well as analysis results are displayed in this field.</td></tr><tr><td>User Feedback</td><td>Shows why the employee reported the email — their selected reason and any comment — giving analysts the reporter's context.</td></tr><tr><td>AI Analysis</td><td>For tenants with an active AI license, an autonomous AI agent investigates the reported email and gives a structured verdict with its risk level, supporting evidence, and recommended actions — so analysts can decide quickly without manual investigation.</td></tr></tbody></table>

#### User Feedback

When an employee reports a suspicious email with the **Page** **View** type of **Keepnet** **Phishing** **Reporter** add-in, they can select a reason and optionally add a comment. The **User Feedback** tab shows this context so analysts understand why the email was reported, directly from the analysis flow.

* **Reason:** The reason the employee selected when reporting the email (for example, "Received spam email").
* **User Comment:** Any additional note the employee wrote when reporting. If none was provided, this shows "No user comment was provided."
