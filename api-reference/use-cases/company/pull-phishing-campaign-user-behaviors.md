...
* **Export per behavior list:** `POST /api/phishing-simulator/phishing-campaign-job-report/{searchType}/search/export/{resourceId}/{instanceGroup}` — e.g. `Opened` with the same `phishingCampaignResourceId` and `instanceGroup`.
