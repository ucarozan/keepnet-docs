...
\{% hint style="info" %\} **Platform UI:** Go to **Company → Company Settings → REST API** and click **+ NEW**. Full instructions: [REST API Settings →](https://doc.keepnetlabs.com/next-generation-product/platform/company/company-settings/rest-api) \{% endhint %\}
...
\{% hint style="warning" %\} **The Client Secret is shown only once.** Copy and store it securely immediately after creation — it cannot be retrieved later. \{% endhint %\}
...
Tokens expire after **1 hour**. There is no refresh token — request a new token using your credentials when the current one expires.
...
\{% hint style="info" %\} To restrict a credential set to specific products (e.g. Awareness Educator only), create a custom role with limited privileges. [Roles and permissions →](https://doc.keepnetlabs.com/next-generation-product/platform/company/system-users) \{% endhint %\}
...
* Create separate credentials per integration — don't share between systems \{% endhint %\}

\{% hint style="danger" %\} **Don't:**

* Hard-code `client_id` or `client_secret` in source code
* Commit credentials to version control \{% endhint %\}

***

### What's next

* [Manage System Users →](https://claude.ai/chat/guides/manage-system-users)
* [Pull Training List →](https://claude.ai/chat/guides/pull-training-list)
